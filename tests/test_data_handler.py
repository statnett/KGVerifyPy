...
def test_load_data_files_nofiles(mock_merge: MagicMock, mock_make: MagicMock) -> None:
    handler = DataHandler()
    handler.data_files = []
...
@patch(f"{PATCH_LOCATION}.make_graphs_from")
@patch(f"{PATCH_LOCATION}.merge_trig_graphs")
def test_load_data_files_formats(mock_merge: MagicMock, mock_make: MagicMock, format: str) -> None:
...
@patch(f"{PATCH_LOCATION}.make_graphs_from")
def test_load_shacl_file(mock_make: MagicMock, shacl_file: str) -> None:
    handler = DataHandler()
...
@patch(f"{PATCH_LOCATION}.make_graphs_from")
def test_load_rdfs_files(mock_make: MagicMock, rdfs_files: list[str]) -> None:
    handler = DataHandler()
