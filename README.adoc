...
**KGValidatePy** is a Python library for validating knowledge graphs in DataOps and ModelOps workflows.

KGValidatePy extends the capabilities of link:https://github.com/RDFLib/pySHACL[pySHACL] and integrates tightly with **KGraphPy**, providing structured validation pipelines for standards-based knowledge graph data, including CIM / CGMES–style datasets.
...
KGValidatePy builds on top of RDFLib-based graph handling while remaining implementation-neutral at the interface level.
...
KGValidatePy does not reimplement SHACL validation logic; it orchestrates and extends it for DataOps and ModelOps use cases.
...
KGValidatePy extends these capabilities for standards-based DataOps and ModelOps workflows but remains an independent open-source project.
