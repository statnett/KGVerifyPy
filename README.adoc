...
Note concerning namespaces: 
The default context uses the standard namespaces for cim and eu:

- cim: "https://cim.ucaiug.io/ns#"
- eu: "https://cim.ucaiug.io/ns/eu#"

However, because these namespaces are assumed to have the same datatype information they are also allowed:
...
If the no violations are found "Conforms: True" is shown.

If violations are found a short summary is given of what kind of violations where found and the number of each. The full result is written
to a graph file.

A CSV report can optionally be saved. This is a reorganised version of the result graph which can be reviewed in Excel.
